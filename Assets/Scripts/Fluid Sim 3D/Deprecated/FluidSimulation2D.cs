...
public class FluidSimulation3D : MonoBehaviour {
    [Header("References")] public ProceduralParticleRenderer3D particleRenderer;
    public ParticleSettings settings;
    public ParticlesSpawner3D particlesSpawner;
    ...
    void Awake() {
        particlesSpawner = new ParticlesSpawner3D(settings);
        if (cameraRef == null)
            cameraRef = Camera.main;
    }

    void Start() {
        particleRenderer = GetComponent<ProceduralParticleRenderer3D>();
    ...
    void UpdateDensities() {
        System.Threading.Tasks.Parallel.For(0, _positions.Length, i => {
            _densities[i] = DensityCalculator3D.CalculateDensity(
